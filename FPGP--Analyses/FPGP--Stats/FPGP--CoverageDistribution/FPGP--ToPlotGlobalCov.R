...
#a0[,2] <- round(a0[,2])
b0 <- data.frame(Coverage = a0$Coverage, Type = "")
  
ggplot(b0, aes(x = Coverage, fill = Type, colour = Type)) +
  geom_density(alpha = 0.15, adjust = 0.75, size = 0.3) +
  theme(legend.position="none") +
  scale_x_continuous("Global Depth (X)",
                     breaks = c(5000, 10000, 15000, 20000, 25000, 30000, 35000, 40000, 45000, 50000, 55000, 60000, 65000, 70000, 75000, 80000, 85000, 90000),
                     labels = c("5K", "10K", "15K","20K", "25K", "30K", "35K", "40K", "45K", "50K", "55K", "60K", "65K","70K", "75K", "80K", "85K", "90K"),
                     expand = c(0,0),
                     limits = c(0, 91000)) +
...
ggsave("FPGP--GoodSamples--GlobalCov--Article--Ultra.jpeg", height=2, width=4, scale=3.5, dpi=300)
