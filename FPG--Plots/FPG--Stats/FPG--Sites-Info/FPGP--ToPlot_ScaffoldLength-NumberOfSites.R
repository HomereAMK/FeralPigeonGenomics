...
setwd("/Users/hintze/Desktop/PhD\ Action\ Plan/Core\ Projects/Finishing/FPGP--FINAL/Analyses/FPGP--Stats/FPGP--SNP-Info/")

library(ggplot2)

a <- read.table("FPGP--GoodSamples--Article--Ultra.ScaffoldInfo_OnlyWithSites.txt")
colnames(a) <- c("Scaffold","ScaffoldLength", "NumberOfSites")

Regression <- lm(formula = a$ScaffoldLength ~ a$NumberOfSites, data = a)
summary(Regression)

ggplot(a,aes(ScaffoldLength, NumberOfSites)) + stat_smooth(method="lm") + geom_point(alpha=0.7, color="#000000", size=1) +
  annotate("text", label = "Multiple R-squared: 0.9905", x = 15000000, y = 130000, size = 5, colour = "#FF0000") +
  annotate("text", label = "P-value: < 2.2e-16", x = 15000000, y = 122500, size = 5, colour = "#FF0000") +
...
ggsave("FPGP--GoodSamples--Article--Ultra.ScaffoldInfo_OnlyWithSites.pdf", height=2, width=4, scale=3.5, dpi = 1000)
