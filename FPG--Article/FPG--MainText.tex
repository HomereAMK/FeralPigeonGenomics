...
\begin{flushleft}
\mycaptions{\textbf{Fig. 1 Sampling effort.} Map showing sampling localities Samples were divided into four categories: a) remote localities within natural range (green); b) urban localities within natural range (yellow), localities outside natural range (orange); d) captives (blue). The supposed natural range of \textit{Columbia livia} is depicted through a light green shade.}
\end{flushleft}
...
\begin{flushleft}
\mycaptions{\textbf{Fig. 2 Maximum-likelihood phylogeny.} Phylogenetic relationships amongst over 450 pigeons representing 45 sampling localities. The outgroup \textit{C. rupestris} is depicted in red. Nodes with bootstrap values above 70\% are marked with green circles.}
\end{flushleft}
...
\mycaptions{\textbf{Fig. 3 Population genetics estimates.} a) Proportion of observed heterozygous sites. b) Nucleotide Diversity ($\pi$). c) Watterson's Theta ($\theta$w). d) Tajima’s D. All absolute values can be found in the Supplementary Spreadsheet.}
...
\mycaptions{\textbf{Fig. 4 Estimation of Admixture proportions.} Individuals are represented by columns, while rows depict the Admixture proportions based on the assumption of different numbers of ancestral populations (K = 2 - 15). Individuals are sorted by sampling locality (light grey upper labels) and grouped per biological status.}
...
\mycaptions{\textbf{Fig. 5 Multidimensional scaling analysis.} Dimensions 1 and 2 are plotted and each point on the plot represents a single individual. The ellipses represent the rough distribution of the two most distinct groups.}
